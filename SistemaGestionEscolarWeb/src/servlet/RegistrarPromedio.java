...
public class RegistrarPromedio extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (codigoCurso.matches("\\d*")){
			...
			//Requerimos el servicio que permite registrar el promedio
			ServicioProfesorProxy profesor = new ServicioProfesorProxy();
			//Enviamos los parametros y almacenamos el resultado del servicio
			resultado = profesor.registrarPromedio(rutEstudiante, idCurso, promedio, rutProfesor);
		...
		request.getRequestDispatcher("/RegistrarPromedios.jsp").forward(request, response);
	}
